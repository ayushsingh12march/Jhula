...
class HelperSocket {
  ...
  static evtLogger(evt) {
    const { detail } = evt;
    HelperSocket.logToConsole(evt.type, 'EVT', detail);
  };
  ...
  registerAllEvents() {
    this.evts.candidate.forEach((evt) => {
      this.skylinkEventManager.addEventListener(evt, HelperSocket.evtLogger);
    ...
    HelperSocket.logToConsole("HelperSocket", `All evts`, 'registered');
  };
  ...
  registerOrUnregisterEvts(evtToToggle) {
    ...
    if (this.registeredEvts[evtToToggle]) {
      this.evts[evtToToggle].forEach((evtType) => {
        this.skylinkEventManager.removeEventListener(evtType, HelperSocket.evtLogger)
      });

      HelperSocket.logToConsole("HelperSocket", `${evtToToggle} evts`, 'unregistered');
      this.registeredEvts[evtToToggle] = false;
      this.registeredEvts.allEvts = this.isAllEvtsRegistered();
    } else {
      if (!this.registeredEvts[evtToToggle]) {
        this.evts[evtToToggle].forEach((evtType) => {
          this.skylinkEventManager.addEventListener(evtType, HelperSocket.evtLogger)
        });

        HelperSocket.logToConsole("HelperSocket", `${evtToToggle} evts`, 'registered');
